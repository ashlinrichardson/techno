...
file_list = open("file_list.txt", "wb") # list of wav files
a = os.system("rm tmp*.wav") # clean up intermediary files
wavs = os.popen("ls -1 *.wav").readlines() # list non-intermediary wav files
wavs = [x.strip() for x in wavs]
...
for i in range(len(visited) - 1):
	j = visited[i]
	f = wavs[j] # filename of song to transition from
	print("read", f)
	samplerate, data = wavfile.read(f)
	L1 = data.shape[0]

	j = visited[i + 1]
	f = wavs[j] # filename of song to transition to!
	print("read", f)
	...
	for dx in range(1, math.floor(drag_width * samplerate), 11): # should parallelize this part!
